...
static const char* kTAG = "dvb-file";
#define ALOGI(...) \
                      ...
                      const struct dvb_parse_file *parse_file)
{
    ...
    if (!dvb_file) {
        ALOGI("Allocating memory for dvb_file");
        return NULL;
    }
...
error:
    ALOGI("ERROR %s while parsing line %d of %s\n",
         err_msg, line, fname);
...
struct dvb_file *dvb_read_file(const char *fname) {
    ...
    return dvb_read_stream(fd);

}
struct dvb_file *dvb_read_stream(FILE* file) {
    ...
    if (entry)
        adjust_delsys(entry);
    fclose(fd);
    ALOGI("Returning %p",dvb_file);
    return dvb_file;

error:
    ALOGI("ERROR %s while parsing line %d\n",
         err_msg, line);
    if (buf)
        free(buf);
    dvb_file_free(dvb_file);
